...
test('renders Admin page ====> Adhérents', () => {
  ...
  const itnLink = screen.getByRole("button", { name: /Mes adhérents/ });
  act(() => {
    userEvent.click(itnLink);
  });

  const heading1 = screen.getByRole('heading', { name: /Mes adhérents/i, level: 1 });
...
test('renders Admin page ====> Les itinéraires', () => {
    ...
    const regLink = screen.getByRole("button", { name: /Les itinéraires/ });
    act(() => {
